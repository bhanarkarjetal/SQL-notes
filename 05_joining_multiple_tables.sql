...
-- to create a summary report where we can view theorder details
-- the customer in the order and the salesman involved
...
EXEC sp_help orders
-- data type of purhase_Date is DATE, there are special actions to perform on DATE data type
-- there are special functions for DATE data type
...
RIGHT OUTER JOIN
	orders
ON
	discounts.disc_year = YEAR(orders.purchase_date)
AND
	discounts.disc_month = MONTH(orders.purchase_date)
...
-- COALESCE (A,B): if A = NULL,then it will be replaced by value B
--				 : if it is not null, then it will return the same A value
...
-- columns that are not the part of AGGREGATE function has to be a part of GROUP BY function
...
LEFT OUTER JOIN
	discounts AS ds
ON 
	ds.disc_year = YEAR(od.purchase_date)
AND
	ds.disc_month = MONTH(od.purchase_date)
INNER JOIN
	salesman AS sm
ON
	od.salesman_id = sm.sales_id
GROUP BY 
	od.salesman_id,
	sm.sales_name
ORDER BY
